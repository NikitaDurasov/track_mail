...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        self.attention_matrix = nn.Linear(self.max_length, self.max_length)
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        attn_weights_new = self.attention_matrix(attn_weights)
        attn_applied = torch.bmm(attn_weights_new.unsqueeze(0),
...
if use_cuda:
    encoder = encoder.cuda(CUDA_DEVICE)
    attn_decoder = attn_decoder.cuda(CUDA_DEVICE)

trainIters(encoder, attn_decoder, 750, print_every=100)
evaluateRandomly(encoder1, attn_decoder1)

with open('ENCODER.pkl', 'wb') as f:
    pickle.dump(encoder, f)
with open('DECODER.pkl', 'wb') as f:
	picke.dump(attn_decoder, f)